...
beta_linear    = X_linear    \ beauty;
beta_quadratic = X_quadratic \ beauty;
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
likelihood = @(X, b, sigma) prod(normpdf(beauty, X*b, sigma));
% or
likelihood = @(X, b, sigma) prod(exp(-(beauty - X*b).^2 ./ sigma) ./ sqrt(2*pi*sigma.^2));
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
prior     = @(b, sigma) 1.0 ./ sigma;
log_prior = @(b, sigma) - log(sigma);
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
log_posterior = @(X, b, sigma) log_likelihood(X, b, sigma) ...
	                         + log_prior(b, sigma);
% -------------------------------------------------------------------------
...
for i = length(yy):-1:1,
	for j = length(xx):-1:1,
% -------------------------------------------------------------------------
		post_plot(i,j) = log_posterior(X_linear, [b1m(i,j); b2m(i,j)], sigma_OLS);
% -------------------------------------------------------------------------
	end%for
end%for
