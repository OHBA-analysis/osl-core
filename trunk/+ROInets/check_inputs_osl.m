function out = check_inputs_osl(oil)
%CHECK_INPUTS	Checks properties of oil.ROInetworks structure
%
% oil = ROInets.check_inputs(oil) checks inputs and
...
%% Basic sanity checks
if ~strcmpi('rest', oil.paradigm),
    error([mfilename ':NotRestingState'], ...
          'ROI network analyses only set up for resting state data currently. \n');
end%if

if ~isfield(oil.source_recon, 'results_fnames') || isempty(oil.source_recon.results_fnames),
    error([mfilename ':SourceReconNotRun'], ...
          'Source reconstruction has not been run. \n');
end%if

if ~isfield(oil, 'ROInetworks'),
    error([mfilename ':NoSettings'], ...
          'Please set ROI network settings in oil.ROInetworks. \n');
end%if

%% Define object
Inputs               = inputParser;
Inputs.CaseSensitive = false;
Inputs.FunctionName  = 'osl_network_analysis';
...
outputDirectoryDefault = fullfile(oil.source_recon.dirname, 'ROInetworks-results');
Inputs.addParamValue('outputDirectory', outputDirectoryDefault, @ischar);

subjectsDefault = 1:length(oil.source_recon.results_fnames);
Inputs.addParamValue('subjectsToDo', subjectsDefault);

Inputs.addParamValue('timeRange', [], timeRangeFcn);

nParallelDefault = 0;
Inputs.addParamValue('nParallelWorkers', nParallelDefault, numericValidFcn);
...
EnvParamsDefault = struct('windowLength', oil.enveloping.window_length, ...
                          'overlap',      oil.enveloping.overlap,       ...
                          'useFilter',    false);
...
if ~isempty(Inputs.Results.timeRange),
    if iscell(Inputs.Results.timeRange),
        assert(isequal(length(Inputs.Results.timeRange),                    ...
                       length(Inputs.Results.subjectsToDo)),                ...
...
% check parcellation file has correct resolution
requiredResolution = oil.source_recon.gridstep;
actualResolution   = str2double(ROInets.call_fsl_wrapper(...
                       sprintf('fslval %s pixdim1',      ...
                               Inputs.Results.spatialBasisSet)));
assert(isequal(requiredResolution, actualResolution), ...
       [mfilename ':ParcFileDimensionFail'], ...
       ['Check that the binary parcel mask has the same dimensions as ', ...
        'the source reconstruction. \n']);


% select files for analysis
reconResultsFiles = cellfun(@(C) fullfile(oil.source_recon.dirname, C),                   ...
                            oil.source_recon.results_fnames(Inputs.Results.subjectsToDo), ...
                            'UniformOutput', false);
                        
fileCheckFcn = @(f) isempty(f) || ...
                    (~exist(f, 'file') && ~exist([f '.mat'], 'file'));
badFile      = cellfun(fileCheckFcn, reconResultsFiles);
if any(badFile), 
    error([mfilename ':EmptyReconFile'], ...
          'File %s is not a valid recon results filename. /n', ...
          reconResultsFiles{find(badFile, 1, 'first')});
end%if

% add any further useful options
Settings                     = Inputs.Results;
Settings.Regularize          = Reg;                                        % incorporate defaults
Settings.reconResultsFiles   = reconResultsFiles;
Settings.dateRun             = datestr(now, 31);
Settings.gridStep            = oil.source_recon.gridstep;
Settings.nSessions           = length(reconResultsFiles);
...
% set root names for each session
if isfield(oil.source_recon, 'session_names') && ~isempty(oil.source_recon.session_names),
    Settings.sessionNames = oil.source_recon.session_names(Settings.subjectsToDo);
else
    for iSession = Settings.nSessions:-1:1,
        Settings.sessionNames{iSession} = sprintf('ROInetworks_subject_%d', ...
