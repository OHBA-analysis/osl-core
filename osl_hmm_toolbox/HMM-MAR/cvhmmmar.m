...
train = options;
train.verbose = options.cvverbose;
train.updateGamma = train.K>1;
...
for fold=1:nfolds
    ...
    for it=1:options.cvrep
        ...
        if options.K > 1
            if options.initrep>0 && strcmp(options.inittype,'HMM-MAR')
                options.Gamma = hmmmar_init(datatr,Ttr,options);
            elseif options.initrep>0 && strcmp(options.inittype,'EM')
                options.nu = sum(T)/200;
                options.Gamma = em_init(datatr,Ttr,options,Sind,options.zeromean);
            else
                options.Gamma = [];
                for in=1:length(Ttr)
                    gamma = rand(Ttr(in)-order,options.K);
        ...
        hmmtr.K = options.K; hmmtr.train = train; 
        ...
        if fe<Fe,
            Fe = fe;
            residualste =  getresiduals(datate.X,Tte,hmmtr.train.Sind,hmmtr.train.order,...
    ...
    elseif options.cvmode==2, mcv = mcv + (sum(Tte) - length(Tte)*order) * cv(fold);
    else mcv(1) = mcv(1) + cv(fold,1); mcv(2) = mcv(2) + (sum(Tte) - length(Tte)*order) * cv(fold,2);
    end
end
