...
%%
% The first line of output shows that there are 276 channels in sensor space, and
% the asterisk shows that this is the selected montage. The MEEG file has 2
% available online montages, and we will select the second one.
% Note that there are 3559 channels, one for each voxel in the 8mm grid
...
% with the number of channels in the MEEG object. You can display the parcellation from within 
% Matlab using the |plot()|  method of the parcellation object. Use the dropdown 
% list to view each of the parcels.
...
% Recall that our original .nii file contained a matrix corresponding to the weighted
% parcellation. Instead of specifying a .nii file, we can specify a matrix of voxel 
% assignments n_voxels x n_parcels. The advantage of this approach is that we can manipulate
% the matrix directly within Matlab to process it before passing it to ROInets.
% Here, we can assemble the parcellation matrix by first binarizing the original parcellation
% (removing the overlap and weights), which yields a binary mask that is 23x27x23x38. We can
% then convert this to the 3559x38 matrix that is required by ROInets. Both of these operations
% are built into the OSL parcellation object.
...
% Because the active montage now has has 38 channels, if we tried to run get_node_tcs again, 
% an error would be thrown because the wrong montage is selected - to repeat |get_node_tcs()|,
% it would be necessary to first run |D = D.montage('switch',2)|
has_montage(D)
...
%%
% And then can plot the amplitude envelope correlation. Note that the correlations along 
...
%%
% However, due to spatial leakage, there are spurious correlations between parcels that
% have been introduced by the beamforming process. If we plot the raw correlation matrix, 
% we can see that there are strong correlations present. These reflect a combination of 
% real correlations, and beamformer-induced correlations. 
figure
...
% This function operates on the D file directly. To use this function, the 
% the active montage needs to be the one with the parcel timecourses
D = D.montage('switch',3);
...
% correlations in the raw timecourses. If we now plot the raw correlation matrix
% for the orthogonalized data, we can verify that no correlations remain.
figure
...
%%
% However, this does not remove correlations in the amplitude envelope, which 
% are typically used as an estimate of MEG functional connectivity. 
...
% regions that are known to be connected. However, there is a trade-off in that reducing the
% number of samples also increases the magnitude of correlations between 
...
% important that you do NOT simply select a subset of the data points e.g. 
% reducing the sampling rate by a factor of 10 should be done with |resample(data,1,10)| 
% and NOT with |data(1:10:end,:)|
...
%%
% Note how the correlations become stronger as the sampling frequency is reduced.
% The extent to which you can downsample your data depends on the quantity of 
% data. Because the recording is only 45 seconds long, downsampling to 1Hz means that the
% correlation is computed using only 45 samples, which is not enough to robustly estimate
% connectivity.
%
