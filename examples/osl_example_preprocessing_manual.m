...
% The script for the practical is osl_example_preprocessing_manual.m which can be found in the osl directory.
% You will need to open this file in a Matlab editor (e.g. type open osl_example_preprocessing_manual
% from the Matlab command window, once osl has been started up). 
...
% 
% We will take an approach here which is run step-by-step and
...
% 
% We will take this fif file and run it through a manual
% preprocessing pipeline as described above.



%% SETUP ANALYSIS
% The only thing you need to do is to go into your OSL directory (i.e. cd /somedirectory/osl-core ) and type
% the following. 
osl_startup;
%%
% This will not be necessary after you have done this once,
% i.e. no need to repeat during one of the follow-up practicals.

%% 
% * SPECIFY DIRECTORIES FOR THIS ANALYSIS
% Datadir and working dir should now be be pointing to the right
% folder, where our example data are stored.
...
%  19 = Midway break 12 = Green fixation cross (response trials) 
...
%%
% Have a look at the SPM object by typing 'D'. Note that this is continuous data, with 232000 time
...
%%
% This will return the number of trials: 
D.ntrials

%%
% Below will show the list of condition labels for each trial:
D.conditions

%%
% This will display a list of unique condition labels:
D.condlist

%%
% Here you will see the order and the names of the channels:
D.chanlabels

%%
% To display the type for each channel, type:
D.chantype
...
%%
% Size is given in number of channels, samples and trials (respectively). The size of each
% dimension separately can be accessed by D.nchannels, D.nsamples and
% D.ntrials. Note that although the syntax of these commands is similar to
% those used for accessing the fields of a struct data type in Matlab what
% is actually happening here is that these commands evoke special functions
% called 'methods' and these methods collect and return the requested
% information from the internal data structure of the D object. The
% internal structure is not accessible directly when working with the
% object. This mechanism greatly enhances the robustness.
...
%%
% For help on any method, type help meeg/method_name to get help about a
% method. In terms of using above methods - you can use it in the way
% shown (e.g. D.ntrials) or in the way you normally use functions, i.e.
% ntrials(D) where D is an argument (might need additional arguments as well).
...
% trends in the data. SPM file names for input are already set, but we repeat this here so this cell can be used independent of the others. The
% filtered data set will get the prefix 'f' preceding the file name.
% Set filenames used in following steps and filter.
...
%%
% In a second step we reduce the line noise around 50 Hz. Please remember that filtering is
% blind to the origin of your signal, it might remove both line noise and
% any other neural signals at the same frequency, for example real gamma
% activity. However, for our practical and the goal of looking at
% event-related fields (ERFs) this approach is sufficient. The order of
% these two filtering procedures should not matter. But as a general rule,
% always double-check your data after filtering to avoid bad surprises
...
% prefix 'd' (preceding any other prefixes acquired before).
S=[];
for subnum=1:length(spm_files), % iterates over subjects
...
% Set filenames used in following steps. Again, to load the latest data
% set, we need the prefixes ffd (2x filtered and downsampled after).
for subnum = 1:length(spm_files), % iterates over subjects
...
%%
% Let's have a look at the SPM object. Note that it is continuous data, with 139200 time
...
% Note that there are some large artefacts. We will use the oslview functionality
% to remove the bad epochs ( see <https://sites.google.com/site/ohbaosl/preprocessing/oslview>
% to get more detailed information about how to use OSLview).
...
% around 325s, 380s and 600s as bad, as well as everything from 650 seconds to the end. Marking is done by right-clicking in
% the proximity of the event and click on 'Mark Event'. A first click
% (green dashed label) marks the begin of a bad period, another second
% click indicates the end (in red). This will mean that we are not using about half of the data. But with such bad
% artefacts this is the best we can do. We can still obtain good results
% with what remains. NB: Push the disk button to save to disk (no prefix will be added, same name is kept).

D=oslview(D);

%% MANUAL AFRICA DENOISING
% In a next step we will run AFRICA denoising. AFRICA uses independent component analysis (ICA) to
...
% artefacts and mains noise can be identified and removed from the data or at least attenuated. In
% this practical we will use manual artefact rejection by looking at the
% time courses and sensor topographies of each component and rejecting
% those that correlate with EOG and ECG measurements. The user interface
% displays the time course, power spectrum and sensor topography for each
% component. These components are sorted based on one of a number of
% metrics, which you can toggle using the dropdown menu. For more
% information about AFRICA and how to use it, please see the AFRICA wiki
% manual on <https://sites.google.com/site/ohbaosl/preprocessing/africa> .
...
%%
% Use the drop-down menu to go to ECG (electrocardiogram, heartbeat-related electrical activity). Shown below is the first component that you should see then. This is the component that is most correlated to the ECG channel
% measurements. Mark this first component which should look like the one below for rejection using the red cross. 
% 
% <<AFRICA_HEARTBEAT.png>>
% 

%%
% Now do the same for the eye movement component, use 'EOG' in the drop-down menu and the first component you see should
% look like the one shown below. Mark this one for rejection as well. Then
% close the AFRICA window to save the changes.
% 
...
% effect this has on our data quality. AFRICA saved the cleaned data by default with 'A' preceding
% all the other prefixes ('ffd' in our case) . However, it also saved an 'online montage' attached to the
...
%%
% These online montages are linear combinations of
% the original sensor data (= montage 0) that can be used to basically represent any linear
% operation. This can be very convenient since it avoids amassing data without need. Here, for example, our first online montage has been generated 
% by AFRICA and represents the data after having removed both the most important ECG and EOG components. 
...
%%
% Now we can switch to any montage we want. D_raw switches to the original, raw data while D_africe switches to the AFRICA denoised
% data. Have a look at the output to see the difference.
D_raw=D.montage('switch',0)
%%
D_africa=D.montage('switch',1)

%%
% Keep in mind that just switching ( as in typing _D.montage('switch',0)_ ) is not enough, you need to assign the switched
% montage a variable. It will switch, but there will be no new object with the online montage applied. So always use it in the way shown above.
% Examine the content of each object by just typing _D_raw_ and _D_africa_
% .

%%
% Now we plot some data to have a look at the differences between raw and denoised
% data.
...
xlim([10 20]);xlabel('Time (s)');
legend({'Raw' 'AFRICA'});
saveas(gcf,['/Applications/osl/osl-core/docs/matlab/HEARTBEAT_DENOISING'],'png')
...
%%
% 
% <<HEARTBEAT_DENOISING.png>>
% 


%%
% The first part of this figure plots the ECG channel included in the
% recording as reference (channel 308). In the second part we plot the two different variants of the same data set, the first  shows the raw time series, and the second shows
% AFRICA run on it and cleaned (EOG / ECG attenuated). You can see that there
% is some commonalities between the ECG and some part of the raw, uncleaned
% time-series in the data (at least in that channel, 306), but that this is gone in the AFRICA denoised version.
...
%% EPOCHING OF DATA
% Now we will do some preliminary epoching for the purpose of finding outliers. This is not
...
% outlier trial rejection (before running the fully automated OAT). 
% 
...
% 
% Here the epochs are set to be from -1000ms to +2000ms relative to the triggers in the MEG
...
%%
% As before, here we set filenames used for the following step. Prefix is now 'dff'.
for subnum = 1:length(spm_files), % iterates over subjects
    spm_files{subnum}=[workingdir '/dff' spm_files_basenames{subnum}];
end
...
% preceding all the other prefixes acquired during preprocessing (in our case
% 'dff').
...
%% 
% This command helps to identify trials of a certain type using the indtrial function. E.g.:
...
% and D.time is used to return the time points relative to the trigger events (t=0) in seconds.
...
saveas(gcf,['/Applications/osl/osl-core/docs/matlab/ERF_NOISY'],'png')
...
saveas(gcf,['/Applications/osl/osl-core/docs/matlab/ERF_NOISY_AVG'],'png')
...
%%
% As you will notice, the ERFs obtained are actually not usable at all.
% However, we should bear in mind that this data is averaging over
% all data including data already marked as bad (using
% oslview). As well as excluding these already marked as bad noisy data
% segments, we can do even better by next performing outlier rejection on
% the now epoched data, and exclude any bad trials or channels (see
% below).
...
% Please note that this operation does not create a new file or new prefix.
...
%%
% Here we switch to montage zero (raw). Sometimes this is necessary, when
...
%%
% Nowe we identify the motorbike image trials. Note that _indtrial_ includes good AND bad
% trials, so bad trials need to be excluded. _Setdiff_ finds the trials that are not bad.
good_motorbike_trls = setdiff(D_raw.indtrial('Motorbike'),D.badtrials)

%%
% As before after AFRICA, we will make use the online montages. Note that the
% online montage got carried over when doing the epoching. So in principle
% there is no need to do epoching on both the 'raw' and AFRICA denoises
% data. Epoching on the raw data containing the AFRICA montage allows you
% to switch between raw and AFRICA denoised data after without problems. So
% now we switch to online montage 1 ('AFRICA denoise data'). Keep in mind
% to assign it a new object (e.g. D_africa)
D_africa=D.montage('switch',1)

%%
% Now let us plot a cleaned rudimentary ERF. After having excluded the bad
% samples from oslview and the rejected trials from the artifact rejection,
% data should look much better. Now, even for the epoched data, the online
...
% denoised data. You should find that now, due to averaging there is less of a
% difference then when looking at continuous data.

%%
% This should give you some nice event-related fields.
figure('units','normalized','outerposition',[0 0 0.4 0.5]); % plots gradiometers
subplot(1,2,1);plot(D_raw.time,squeeze(mean(D_raw(planars(135),:,good_motorbike_trls),3))); % raw data
xlabel('Time (seconds)','FontSize',20);ylim([-10 6])
set(gca,'FontSize',20)
ylabel(D.units(planars(1)),'FontSize',20);
hold on;
subplot(1,2,1);plot(D_raw.time,squeeze(mean(D_africa(planars(135),:,good_motorbike_trls),3))); % AFRICA denoised
xlabel('Time (seconds)','FontSize',20);ylim([-10 6])
...
plot(D_raw.time,squeeze(mean(D_raw(magnetos(49),:,good_motorbike_trls),3))); % raw data
xlabel('time (seconds)','FontSize',20);ylim([-300 500])
ylabel(D.units(magnetos(1)),'FontSize',20);
set(gca,'FontSize',20)

hold on;
subplot(1,2,2);plot(D_africa.time,squeeze(mean(D_africa(magnetos(49),:,good_motorbike_trls),3)));% AFRICA denoised
xlabel('time (seconds)');ylim([-300 500])
ylabel(D.units(magnetos(1)),'FontSize',20);
set(gca,'FontSize',20)
legend({'Raw' 'AFRICA'})

saveas(gcf,['/Applications/osl/osl-core/docs/matlab/ERF_CLEANED_AVG'],'png')
...
saveas(gcf,['/Applications/osl/osl-core/docs/matlab/ERF_CLEANED_AVG_ALL_SENSORS'],'png')

%%
% These 2D images should correspond to the curves before. They should look like
% this:

%%
% 
% <<ERF_CLEANED_AVG_ALL_SENSORS.png>>
% 


%%
% * PLOTTING EVENT-RELATED TOPOGRAPHIES AT DEFINED LATENCIES *
% To plot a cleaned rudimentary ERF topography (here at a relatively late
% latency) over all good trial, do:

topo=squeeze(mean(D(:,188,good_motorbike_trls),3));

figure('units','normalized','outerposition',[0 0 0.4 0.5]);
sensors_topoplot(D,topo,{'MEGPLANAR' 'MEGMAG'},1);set(gco,'FontSize',20)
saveas(gcf,['/Applications/osl/osl-core/docs/matlab/TOPO_CLEANED_AVG_ALL_SENSORS'],'png')
...
%%
% 
% <<TOPO_CLEANED_AVG_ALL_SENSORS.png>>
% 
...
saveas(gcf,['/Applications/osl/osl-core/docs/matlab/TOPO_CLEANED_AVG_ALL_SENSORS_AFRICA'],'png')

%%
% They should look like
% this. Have a look at how different these topographies look. Do you have
% an idea why now everything looks much more similar than before?

%%
% 
% <<TOPO_CLEANED_AVG_ALL_SENSORS_AFRICA.png>>
% 
