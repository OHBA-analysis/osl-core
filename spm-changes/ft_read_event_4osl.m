function [event] = ft_read_event_4osl(filename, varargin)
...
fixoxfordneuromag    = keyval('fixoxfordneuromag', varargin); % MWW OSL2
  ...
  case {'neuromag_fif' 'neuromag_mne' 'neuromag_mex'}
    ...
    if iscontinuous
      ...
      if ~isempty(binaryindx)
        ...
        trigger = read_trigger_4osl(filename, 'header', hdr, 'dataformat', dataformat, 'begsample', flt_minsample, 'endsample', flt_maxsample, 'chanindx', binaryindx, 'detectflank', detectflank, 'trigshift', trigshift, 'fixneuromag', 0, 'fixoxfordneuromag', fixoxfordneuromag); % MWW OSL1.1 - corresponding change in read_trigger
        event   = appendevent(event, trigger);
