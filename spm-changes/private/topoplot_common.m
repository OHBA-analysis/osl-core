...
if strcmp(cfg.interactive, 'yes')
  ...
  guidata(gcf, info);
  % attach data to the figure with the current axis handle as a name
  %dataname = num2str(gca);%MWW
  %dotpos   = findstr(dataname,'.');%MWW
  dataname = ['DATA'];% dataname(1:dotpos-1) 'DOT' dataname(dotpos+1:end)];%MWW
  tmp=gcf;%MWW
  setappdata(tmp.Number,dataname,varargin(1:Ndata));%MWW
...
%  dotpos   = findstr(dataname,'.');%MWW
  dataname = ['DATA'];% dataname(1:dotpos-1) 'DOT' dataname(dotpos+1:end)];%MWW
  tmp=gcf;%MWW
  data = getappdata(tmp.Number, dataname);%MWW
