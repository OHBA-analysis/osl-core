...
if is_sensor_space,                   
    ...
    for cc=1:length(first_level_cons_to_do),
        ...
        con=first_level_cons_to_do(cc);

        disp(['Processing Group stats for first level contrast ' num2str(con) ', ' oat.first_level.contrast_name{con}]);
        ...
        con_report_dir=[group_level_report.dir '/c' num2str(con) ];
        con_report=osl_report_setup(con_report_dir,['First level contrast ' num2str(con) ', ' oat.first_level.contrast_name{con}]); 
        ...
        if ~oat.group_level.space_average,
            ...
            for gconi=1:length(group_level_cons_to_do),  
                ...
                if(nfreqs>1),                
                    ...
                    fig_name{1}=['stats_at_max_multiplot_c' num2str(con) '_gc' num2str(gcon)];                      
                    fig_title{1}=['Stats for gc' num2str(gcon) ' [' oat.group_level.group_contrast_name{gcon} ']']; 
                    fig_title{1}=[fig_title{1} ' (Max found using c' num2str(first_level_cons_to_do(1)) ', gc' num2str(group_level_cons_to_do(1)) ')'];
    ...
    if(oat.group_level.report.show_lower_level_copes)
        ...
        for coni=1:length(first_level_cons_to_do),  
            ...
            fig_name{coni}=['lower_level_stdcope' num2str(con) '_gc' num2str(con)];  
            fig_title{coni}=['Average Lower level STDCOPEs for c' num2str(con) ' [' oat.first_level.contrast_name{con} ']'];  
            %subplot(1,2,1);
    ...
    if(ntpts>1)
        ...
        if(oat.group_level.report.show_lower_level_copes && length(group_level_results.frequencies)==1)
            ...
            for coni=1:length(first_level_cons_to_do),  
                ...
                fig_title{coni}=['Lower level tstats for c' num2str(con) ' [' oat.first_level.contrast_name{con} '], at t=' num2str(group_level_results.times(time_ind_max)) 'secs, f=' mat2str(group_level_results.frequency_ranges(freq_ind_max,:)) 'Hz'];  
                fig_title{coni}=[fig_title{coni} ' (Max found using c' num2str(first_level_cons_to_do(1)) ', gc' num2str(group_level_cons_to_do(1)) ')'];
                %subplot(1,2,1);
                ...
                plot(group_level_results.times,squeeze(mean(group_level_results.lower_level_copes{con}(vox_ind_max,:,:)./group_level_results.lower_level_stdcopes{con}(vox_ind_max,:,:),2)),'LineWidth',4);
            ...
            for coni=1:length(first_level_cons_to_do),  
                ...
                fig_title{coni}=['Lower level STDCOPEs for c' num2str(con) ' [' oat.first_level.contrast_name{con} '], at t=' num2str(group_level_results.times(time_ind_max)) 'secs, f=' mat2str(group_level_results.frequency_ranges(freq_ind_max,:)) 'Hz'];  
                fig_title{coni}=[fig_title{coni} ' (Max found using c' num2str(first_level_cons_to_do(1)) ', gc' num2str(group_level_cons_to_do(1)) ')'];
                %subplot(1,2,1);
