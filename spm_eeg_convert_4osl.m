...
if ~isfield(S, 'fixoxfordneuromag'),     S.fixoxfordneuromag = [] ;                      end% MWW OSL2 - note that there are corresponding changes in ft_read_event and read_trigger
...
if ~isfield(S, 'mode') || ~isequal(S.mode, 'header')
    ...
    S1.fixoxfordneuromag = S.fixoxfordneuromag; %MWW
    Dhdr              = spm_eeg_convert_4osl(S1); %MWW
    hdr               = Dhdr.hdr;
    event             = Dhdr.events;
else
    ...
    try
        ...
        event = ft_read_event_4osl(S.dataset, 'detectflank', 'both', 'eventformat', S.inputformat,'fixoxfordneuromag',S.fixoxfordneuromag);% MWW OSL2 - see corresponding changes in ft_read_event and read_trigger
...
%TC changed to a swtich statemnet
%added 'ctf_grads' case
%when this case is ture, read just CTF grads, trigger and clock channels
switch S.channels
    case 'all'
    ...
    case 'ctf_grads'
        %hdr.chantype: 'trigger' 'headloc' 'adc' 'unknown' 'meggrad' 'refgrad'
        %'refmag', for elekta converted data, there should be megmag as
        %well but this is not being taken care of currently
        %the clock channel is labeled 'unknown' I don't think it's really
        %needed but to be consistent with previous code I will add it
        %manually by searching for the clock name in hdr.label
        
        %find the stim channel
        [~,chansel_stim]=spm_match_str('trigger',hdr.chantype);
        %find the clock channel
...
if isempty(outfile), outfile = 'spm12'; end
