...
for j = 1:length(S)
    ...
    close(h);
	[sss_fif,bad_segments,headpos_file] = osl_maxfilter(S(j).raw,'sss','badchannels',D_temp.chanlabels(D_temp.badchannels),'verbose',true,'fif_out',fullfile(S(j).import,['sss_',S(j).raw]));
