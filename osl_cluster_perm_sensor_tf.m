function [corrp group_tstats dmps] = osl_cluster_perm_sensor_tf(S)

% [corrp group_tstats group_copes] = osl_cluster_perm_sensor_tf(S)
%
% Options:
...
dims=size(bw);
[imlabel LL] = ft_findcluster(reshape(bw, [dims(1) prod(dims(2:end))]),channeighbstructmat,minnbchan);
